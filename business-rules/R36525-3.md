...
| Priority | Normal |
| Modify | NO |
| Action | If (PropertyValueField != Null) AND (ValueId != Null) AND (Property.LimitToAllowedValues == True) <br></br> Validates that: ValueId exists among the property's allowed values list. Otherwise - throw an error (Error1) <br></br> PropertyValueField's value is equal to the value of the allowed value. Otherwise- throw an error (Error2).|
| Description | |
