...
| Description | The rule ensures that only properties with valid values can be saved. A valid property value means that its id is among the property’s allowed values and that the value matches the allowed value corresponding to its id. |
