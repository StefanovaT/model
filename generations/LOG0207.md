...
StoreTransactionLine.AllowOverExecution = If (DocumentFullfimenLineId == StoreOrder.DocumentFullfilmentId ) AND (StoreOrderLine.StandardQuantityBase < All(DocumentFullfillmentLine.StandardQuantityBase)) {AllowOverExecution = True}, else {AllowOverExecution = False}
```
