...
StoreTransactionLine.AllowOverExecution = If (DocumentFullfiment.ParentFullfilment == StoreOrder.DocumentFullfilment) AND (StoreOrderLine.StandardQuantityBase < All(DocumentFullfillment.StandardQuantityBase)) {AllowOverExecution = True}, else {AllowOverExecution = False}
```
